...
from ..features.brep_features import BRepNetExtractor
...
def extract_brepnet_data_from_step(
    ...
    if use_many_threads:
        ...
        for file in tqdm(files):
            extract_brepnet_features(file, output_path, feature_schema)
    # Вычисление статистик стандартизации для B-repNet
...
from ..config import PROCESSED_DATA_DIR, EXTERNAL_DATA_DIR

# command python -m src.pipelines.build_features
...
def run(
    ...
    output_path = step_path_dir.parent / "features/brep" 
