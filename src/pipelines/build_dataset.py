...
# команда для запуска python -m src.pipelines.build_dataset

@app.command()
def run(
    brepnet_dir: Path = typer.Option(PROCESSED_DATA_DIR / 'features' / 'brep', help="Путь к папке с признаками B-repNet (*.npz)."),
    output_file: Path = typer.Option(PROCESSED_DATA_DIR / 'dataset_stats.json', help="Путь к выходному JSON файлу набора данных."),
    validation_split: float = typer.Option(0.2, help="Доля обучающих данных для валидации."),
    test_split: float = typer.Option(0.1, help="Доля всех данных для тестирования."),
    ...
    logger.info(f"Найдено {len(stems)} моделей.")
    ...
    logger.success(f"\nФайл набора данных успешно создан: \n {output_file}")
